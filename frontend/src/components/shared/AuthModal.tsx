...
import { z } from 'zod';
import { X, Mail, Lock, User, Phone, Eye, EyeOff, Loader2, Sparkles, ShieldCheck } from 'lucide-react';
...
export default function AuthModal({ isOpen, onClose }: AuthModalProps) {
  ...
  // Use Zustand store for login and loading state (for the UI buttons)
  ...
  const handleRegister = async (data: RegisterForm) => {
    try {
      ...
      await authService.registerSave({
        email: data.email,
      ...
      const serverMsg = err.response?.data?.message || 'Đăng ký thất bại, email có thể đã tồn tại';
  ...
  const handleVerifyOtp = async (data: OtpForm) => {
    try {
      setIsProcessing(true);
      // Verify OTP
      await authService.verifyOtp(pendingEmail, data.otp);
      // Finalize Registration
      await authService.registerConfirm(pendingEmail);
      toast.success('Xác thực và tạo tài khoản thành công! Đang đăng nhập...');
      
      // Auto login
      await handleLogin({ email: pendingEmail, password: pendingPassword });
      
      // Clean up
      registerForm.reset();
      otpForm.reset();
    } catch (err: any) {
      const serverMsg = err.response?.data?.message || 'Mã OTP không hợp lệ hoặc đã hết hạn';
      toast.error(serverMsg);
    } finally {
      setIsProcessing(false);
    }
  ...
  const switchTab = (newTab: 'login' | 'register' | 'otp') => {
    setTab(newTab);
            ...
            <div className="relative z-[1] p-8">
              ...
              <div className="text-center mb-6">
                ...
                <h2 className="text-2xl font-bold">
                  ...
                  {tab === 'otp' && 'Xác thực Email'}
                </h2>
                <p className="text-sm text-text-secondary mt-1">
                  {tab === 'login' && 'Chào mừng bạn trở lại FastFix!'}
                  {tab === 'register' && 'Tham gia cộng đồng sửa chữa #1 Đà Nẵng'}
                  {tab === 'otp' && `Chúng tôi đã gửi mã OTP gồm 6 chữ số đến ${pendingEmail}`}
                </p>
              </div>

              {/* Tabs (Only show if not in OTP step) */}
              {tab !== 'otp' && (
              ...
              {tab === 'register' && (
                ...
                >
                  <div>
                    ...
                    {registerForm.formState.errors.fullName && <p className="text-xs text-red-400 mt-1">{registerForm.formState.errors.fullName.message}</p>}
                  </div>
                  ...
                  >
                    {isFormLoading ? <Loader2 size={18} className="animate-spin" /> : <>Tiếp tục</>}
                  </button>
                </motion.form>
              )}
              ...
              {tab === 'otp' && (
                ...
                >
                  <div>
                    <label className="block text-xs font-semibold text-text-secondary mb-1.5 text-center">Nhập mã 6 số từ Email</label>
                    <div className="relative flex justify-center">
                      <ShieldCheck size={20} className="absolute left-5 top-1/2 -translate-y-1/2 text-primary" />
                      <input
                        {...otpForm.register('otp')}
                        type="text"
                        maxLength={6}
                        placeholder="000000"
                        className="w-full max-w-[200px] text-center font-mono tracking-[0.5em] text-xl bg-white/5 border border-white/10 rounded-xl py-3 outline-none focus:border-primary/50 focus:shadow-[0_0_20px_rgba(37,99,235,0.2)] transition-all placeholder:text-text-secondary/50"
                      />
                    </div>
                    <div className="text-center">
                       {otpForm.formState.errors.otp && (
                         <p className="text-xs text-red-400 mt-2">{otpForm.formState.errors.otp.message}</p>
                       )}
                    </div>
                  </div>
                  ...
                  <button
                    type="submit"
                    disabled={isFormLoading}
                    className="w-full flex items-center justify-center gap-2 py-3.5 rounded-xl bg-gradient-to-br from-green-500 to-emerald-600 text-white font-bold text-sm shadow-lg shadow-emerald-500/30 hover:scale-[1.01] active:scale-[0.99] transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {isFormLoading ? <Loader2 size={18} className="animate-spin" /> : <>Xác thực & Hoàn tất</>}
                  </button>
