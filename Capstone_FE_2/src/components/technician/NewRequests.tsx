...
const getEtaMinutes = (raw: any) => {
  if (raw === null || raw === undefined || raw === '') return 0;
  const n = Number(typeof raw === 'string' ? raw.replace(',', '.').trim() : raw);
  return Number.isFinite(n) && n > 0 ? n : 0;
};

const formatEtaLabel = (etaRaw: any, baseRaw: any) => {
  const minutes = getEtaMinutes(etaRaw);
  if (!minutes) return 'Đang cập nhật';
  const base = new Date(baseRaw || Date.now());
  if (Number.isNaN(base.getTime())) return `${minutes} phút`;
  const start = new Date(base.getTime() + minutes * 60 * 1000);
                        ...
                        </div>
                        {
                          (() => {
                            ...
                            const etaRaw =
                              ...
                              (request as any)?.eta ??
                              (request as any)?.ETA;
                            const etaLabel = formatEtaLabel(etaRaw, createdRaw);
                            return (
