...
import { 
  ...
  ZoomIn, X, Image as ImageIcon, Zap
} from 'lucide-react';
...
export default function TechAcceptedRequestsPage() {
  ...
  const confirmCancel = async () => {
    if (!user?.id || !cancelOrderId || !cancelReason.trim()) return;
    setCancelLoading(true);
    try {
      await technicianOrderService.rejectOrder({ orderId: cancelOrderId, technicianId: user.id, reason: cancelReason });
                    ...
                    <div className="flex flex-col xl:flex-row gap-8">
                          ...
                          <div className="flex items-center gap-4">
                             ...
                             <div>
                               ...
                               <p className="text-sm font-bold text-emerald-400">
                                 {request.phoneNumber || (request as any).customerPhone || detailsMap[request.orderId]?.customerPhone || 'Chưa có SĐT'}
                               </p>
                             </div>
                          </div>
                       ...
                       <div className="flex flex-row xl:flex-col items-center justify-end gap-3 shrink-0">
                          <button 
                            ...
                            className="w-full sm:w-auto px-8 py-4 bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-black text-xs uppercase tracking-widest rounded-2xl flex items-center justify-center gap-2 transition-all shadow-xl shadow-emerald-500/20 active:scale-95 disabled:opacity-50"
                          >
                            Bắt đầu ngay <Zap size={16} />
                          </button>
                          <div className="flex w-full gap-3">
                             <button 
                               onClick={() => handleCancel(request.orderId)}
                               className="flex-1 px-4 py-3.5 bg-rose-500/10 hover:bg-rose-500 hover:text-white text-rose-400 font-black text-[10px] uppercase tracking-widest rounded-2xl border border-rose-500/30 transition-all active:scale-95"
                             >
                               Hủy đơn
                             </button>
                             <button 
          ...
          >
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-base font-black text-white tracking-tight">Hủy yêu cầu</h3>
                </div>
              </div>
              <button
                onClick={() => setShowCancelModal(false)}
            ...
            <div className="space-y-4">
              <div>
                <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest block mb-2">
                  Lý do hủy yêu cầu <span className="text-rose-500">*</span>
                </label>
                <textarea
                  value={cancelReason}
              ...
              <div className="flex gap-3">
                ...
                >
                  {cancelLoading ? <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : 'Xác nhận hủy'}
                </button>
              </div>
            </div>
          </motion.div>
